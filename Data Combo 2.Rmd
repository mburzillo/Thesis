...
potential_tracts_2 <- rbind(potential_control, potential_treatment) %>%
  ...
  select(-GISJOIN2, -tract_area, -SHAPE_LEN, -intersect_area, -perimeter, -NHGISST_2, -PLACE) %>%
...
sum(big_join$holc_coverage_tracts, na.rm = T)
sum(as.numeric(big_join$selected_tracts), na.rm = T)
```
...
big_join_demo <- big_join_demo %>%
  select(gisjoin_1960:AREANAME, tot_persons_printed:owner_occ_property_value)
...
big_join_demo_2 %>%
  filter(selected_tracts == 1, pop_density < 953.4427, selected_into_treatment == 1)

big_join_demo_3 <- big_join_demo_2[!(big_join_demo_2$selected_tracts == 1 & big_join_demo_2$pop_density < 952.4427),]

subset(big_join_demo_2, selected_tracts == 1 & big_join_demo_2$pop_density < 952.4427)
