...
# add back in extra city pops for 1930 that weren't in tracts selected
additional_pop <- read_csv("1930 - 1940 City Population Data/1930_data_ipums_full_sample_collapsed.csv") %>%
  select(numeric_city_code = city, citypop_1930 = citypop) %>%
  mutate(citypop_1930 = citypop_1930 * 100)
...
data_for_analysis <- left_join(data_for_analysis, state_ab_region)

data_for_analysis <- left_join(data_for_analysis, additional_pop)
...
big_join %>%
  filter(is.na(area_accounted_for_1960), selected_into_treatment == 1)
```
...
# value of property from printed report (all housing units)
...
big_join_demo$pct_non_white_printed <- (big_join_demo$black_printed +
                                      big_join_demo$other_printed)/(big_join_demo$black_printed +
                                      big_join_demo$other_printed + big_join_demo$white_printed) * 100
...
big_join_demo$pct_owner_occupied <- 100 * big_join_demo$owner_occupied/(big_join_demo$owner_occupied + big_join_demo$renter_occupied)


# pct renter occupied (occupied year round h units)
big_join_demo$pct_renter_occupied <- 100 * big_join_demo$renter_occupied/(big_join_demo$owner_occupied + big_join_demo$renter_occupied)
...
# avg_people_per_room
# Occupied Units by Persons per Room (occupied units)
big_join_demo$avg_occ_per_room_1960 <- (big_join_demo$persons_per_room_half_or_less * .25 + 
                                    big_join_demo$persons_per_room_point51_point75 * (.75+.51)/2 + 
                                    big_join_demo$persons_per_room_point76_one * (1 +.76)/2 + big_join_demo$persons_per_room_onepone_or_more * 1) /(big_join_demo$persons_per_room_half_or_less + big_join_demo$persons_per_room_point51_point75 + big_join_demo$persons_per_room_point76_one + big_join_demo$persons_per_room_onepone_or_more)
...
big_join_recode <- big_join_final %>% 
  ...
  mutate(log_property_value_1960 = log(weighted_prop_value_1960),
         ...
         log_owner_occ_property_value_1960 = log(owner_occ_property_value_1960),
         ...
         citypop_1940 = citypop_1940/10000,
         city_persons_p_hh_1940 = citypop_1940 * 10000/city_hh_1940)
...
```
```{r}
# rejoin area accounted for because seems to be missing for some reason
...
```{r visualizing distribution of pct_grade_D, include = FALSE}
causal_training_sub <- big_join_recode %>%
  filter(low_density_1960 != 1, area_accounted_for_1960 >= .5, holc_coverage_tracts == 1, citypop_1930 >= 60000)

causal_training_sub %>%
  ggplot(aes(x = pct_grade_d)) +
  geom_histogram(bins = 50) +
  theme_bw()+
  ggtitle("Distribution of the HOLC Grade D Coverage Score among 1940s Tracts in the\nTraining Group") +
  xlab("HOLC Grade D Coverage Score") +
  ylab("Number of 1940s Census Tracts") +
  labs(caption = "This plot shows the distribution of the HOLC Grade D Coverage Score, which denotes the percent of a 1940\ncensus tract's area that is covered by an HOLC 'D'/'Red' graded area, for all 1940 census tracts in the\nHOLC grade prediction model training group, which consists of all tracts in HOLC-surveyed cities with 1930\npopulations above 60,000 with at least 50% area overlap with an HOLC-graded area. The distribution is\nheavily concentrated at the extremes of zero and 100.") +
  theme(plot.caption = element_text(hjust = 0, size = 10)) 

ggsave("causal_training_d_hist.png",
       plot = last_plot(),
       device = png(),
       width = 7.5, height =6)
```
...
```{r visualizing distribution of pct_grade_D, include = FALSE}
big_join_recode %>%
  filter(low_density_1960 != 1, area_accounted_for_1960 >= .5, holc_coverage_tracts == 1, citypop_1930 >= 40000, citypop_1930 <= 60000) %>%
  ggplot(aes(x = pct_grade_d)) +
  geom_histogram() +
  theme_bw()+
  ggtitle("Distribution of the % HOLC Grade D Score among 1940s Tracts in the Training\nGroup") +
  xlab("% HOLC Grade D Score") +
  ylab("Number of 1940s Census Tracts")
...
causal_full_group_sub <- big_join_recode %>%
  filter(tot_pop_1940 != 0)

```

```{r seeing if correct tracts are being selected, include = FALSE}

desired_tracts <- big_join_recode %>%
  filter(area_accounted_for_1960 >= .5, citypop_1930 >= 40000, citypop_1930 <= 60000)

selected_by_mechanism <- big_join_recode %>%
  filter(selected_into_treatment == 1, low_density_1960 == 0)

# 86 tracts in desired tracts that are not in selected tracts
length(setdiff(desired_tracts$gisjoin_1960, selected_by_mechanism$gisjoin_1960))
# 266 tracts in selected group that are not in deisred tracts
length(setdiff(selected_by_mechanism$gisjoin_1960, desired_tracts$gisjoin_1960))

# 338 n both lists
length(intersect(desired_tracts$gisjoin_1960, selected_by_mechanism$gisjoin_1960))

full_join(desired_tracts, selected_by_mechanism)

338/424

# 242 with > 75% coverage
44/242
...
# for some reason, area accounted for is not joining

big_join_recode %>%
  mutate(count = 1) %>%
  filter(selected_tracts == 1, low_density_1960 == 0) %>%
  group_by(selected_into_treatment, holc_city, citypop_1930) %>%
  summarise(tot = sum(count), citypop = mean(citypop_1930)) %>%
  filter(selected_into_treatment == 0) %>%
  summarise(mean(tot), mean(citypop))



treat <- big_join_recode %>%
  mutate(count = 1) %>%
  filter(selected_tracts == 1, low_density_1960 == 0) %>%
  group_by(selected_into_treatment, holc_city, citypop_1930) %>%
  summarise(tracts_in_city = sum(count), citypop_1930 = mean(citypop_1930), citypop_1990 = mean(citypop_1990)) %>%
  mutate(pct_growth_30_90 = 100 * (citypop_1990 - citypop_1930)/citypop_1930) %>%
  filter(selected_into_treatment == 1)

control <- big_join_recode %>%
  mutate(count = 1) %>%
  filter(selected_tracts == 1, low_density_1960 == 0) %>%
  group_by(selected_into_treatment, holc_city, citypop_1930) %>%
  summarise(tracts_in_city = sum(count), citypop_1930 = mean(citypop_1930), citypop_1990 = mean(citypop_1990)) %>%
...
mean(treat$citypop_1930)
mean(control$citypop_1930)
mean(treat$pct_growth_30_90)
mean(control$pct_growth_30_90)
```

```{r}

colnames(big_join_recode)
```
