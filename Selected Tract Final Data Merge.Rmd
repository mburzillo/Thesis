...
growth_rate_radii_data <- read_xlsx("treatment_control_growth_rate.xlsx") %>%
  select(-treatment)
...
potential_tracts_demo <- left_join(potential_tracts, city_names_pops_growth, by = c("cityname" = "cityname", "state" = "state"))
...
potential_tracts_demo %>%
  filter(cityname == "Columbus", NHGISST_2 == 130)

potential_tracts_demo %>%
  filter(state == "Alabama")

growth_rate_radii_data %>%
  filter(state == "CA")

growth_rate_radii_data %>%
  filter(city == "Port Arthur")
...
potential_tracts_demo <- potential_tracts_demo %>%
  select(state:AREANAME, tot_persons_printed:weighted_cash_contract_rent_lt_50)
...
tracts_selected <- potential_tracts_demo %>%
  mutate(pop_density >= 952.4427)

potential_tracts_demo %>%
  filter(pop_density >= 1417.682)
