...
ipums_1940_city <- full_join(ipums_1940_hh_level, ipums_1940_person_level) %>%
  select(
    ...
    city_1940_pct_non_white = pct_non_white_perwt,
    ...
    city_1940_avg_y_school_perwt = avg_y_school_perwt
  )
...
tracts_selected <- tracts_selected %>% 
  select(
    ...
    city_1940_pct_non_white,
    city_1940_pct_emp_males_14_plus,
    city_1940_pct_completed_hs,
    city_1940_avg_y_school = city_1940_avg_y_school_perwt,
...
```{r}
# load all data with predictions from stata
test_tracts_selected <- read.csv("Stata Work/test_tracts_selected.csv")
```

```{r}
# find HOLC tracts in range with correct density and >50% areas: 87 total
supposed_to_have <- test_tracts_selected %>%
  filter(holc_coverage_tracts == 1, !(is.na(area_accounted_for_1960)), low_density_1960 == 0,
         area_accounted_for_1960 > .5, !(is.na(predicted_pct_d)),
         citypop_1930 <= 60000, citypop_1930 >= 40000, pct_grade_d >= 50) %>%
  filter(holc_city != "Saint Petersburg", holc_city != "Chelsea", holc_city != "Council Bluffs",
         holc_city != "Malden", holc_city != "Woonsocket")
  view()

# find tracts actually selected into treatment
treat_selected_gis <- test_tracts_selected %>%
  filter(selected_into_treatment == 1, redlined == 1, !(is.na(predicted_pct_d))) %>%
  unique()

sub_all_tracts <- all_holc_tract_grades %>% 
  select(GISJOIN, prop_D_of_accounted_for, prop_grade_D, prop_grade_C, prop_grade_C_D, area_accounted_for) %>%
  unique()

treat_selected_gis_2 <-left_join(treat_selected_gis, sub_all_tracts, by = c("gisjoin_1960" = "GISJOIN"))


# in both
length(intersect(supposed_to_have$gisjoin_1960, treat_selected_gis$gisjoin_1960))

# 11 supposed to be selected but not
length(setdiff(supposed_to_have$gisjoin_1960, treat_selected_gis$gisjoin_1960))
# 54 in selected treatment that are not in test tracts
length(setdiff(treat_selected_gis_2$gisjoin_1960, supposed_to_have$gisjoin_1960))

13/104 * 100

not_selected <- setdiff(supposed_to_have$gisjoin_1960, treat_selected_gis$gisjoin_1960)
incor_select <- c(setdiff(treat_selected_gis_2$gisjoin_1960, supposed_to_have$gisjoin_1960))

length(incor_select)

treat_selected_gis_2 %>%
  filter(gisjoin_1960 %in% incor_select, !(is.na(predicted_pct_d))) %>%
  select(gisjoin_1960, holc_state, holc_city, pct_grade_d, predicted_pct_d, area_accounted_for_1960) 

supposed_to_have %>%
  filter(gisjoin_1960 %in% not_selected , !(is.na(predicted_pct_d))) %>%
  select(gisjoin_1960, holc_state, holc_city, pct_grade_d, predicted_pct_d, area_accounted_for_1960, tract_area_1960)  %>%
  mutate(large = ifelse(tract_area_1960 > 1164947, 1, 0)) %>%
  summarise(mean_D = mean(pct_grade_d), mean_predicted = mean(predicted_pct_d),
            med_D = median(pct_grade_d), median_predicted = median(predicted_pct_d))


# G0600850G0049 obscure
# G0400130PH0094 mostly red
# G0400130PH0094 right next to red
# G06001900011 next to red
# G08010100021  next to red
# G39001700003 mostly red

median(test_tracts_selected$tract_area_1960)

1164947

treat_selected_gis_2 %>%
  select(holc_state, holc_city, prop_D_of_accounted_for, prop_grade_D, prop_grade_C, prop_grade_C_D, area_accounted_for) %>%
  mutate(Cd_ACCOUNT =prop_grade_C_D/area_accounted_for) %>%
  summarise(mean_D = mean(prop_grade_D, na.rm = T), mean_CD = mean(prop_grade_C_D, na.rm = T), mean_accounted = mean(area_accounted_for, na.rm = T), mean_d_acc = mean(prop_D_of_accounted_for, na.rm = T), mean_Cd_ACCOUNT = mean(Cd_ACCOUNT, na.rm = T))

treat_selected_gis_2 %>%
  filter(is.na(area_accounted_for))

```


```{r}
test_tracts_selected %>%
  filter(gisjoin_1960 == "G1302150BC0110")
```
...
test_tracts_selected %>%
  ...
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_black_1960, color = as.factor(selected_into_treatment))) +
  geom_point() 

test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_non_white_1960, color = as.factor(selected_into_treatment))) +
  geom_point() 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_hs_grad_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = weighted_tot_years_school_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_employed_out_of_all_males_19, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_unemployed_males_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = log_property_value_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = log_contract_rent_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_owner_occupied_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_single_unit_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_vacant_1960, color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_housing_deteriorated_dilap_1 , color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_plumbing_fuctional_1960 , color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1) %>%
  ggplot(aes(x = citypop_1930, y = pct_one_point_0_one_people_per_r , color = as.factor(selected_into_treatment))) +
  geom_point() +
  geom_smooth(method = lm, se = F) 
...
test_tracts_selected %>%
  mutate(new_predict = ifelse(predicted_pct_d > 100, 100, predicted_pct_d)/100,
         `Treatment Status` =  as.factor(selected_into_treatment)) %>%
  filter(selected_tracts == 1) %>%
#  filter(redlined == 1) %>%
  ggplot(aes(x = new_predict, color = `Treatment Status`)) +
  geom_density() + 
  scale_color_manual(values=c("black", "red")) +
  ggtitle("Distribution of Predicted Probability of Redlining: Treatment and Control Group") +
  geom_vline(xintercept = .50, linetype = "dashed") +
  xlab("Probability of Being Redlined") +
  ylab("Density") +
  theme_bw()

```

```{r tract level summary stats}
# all selected
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  select(pct_black_1960, pct_owner_occupied_1960, pct_unemployed_males_1960, log_contract_rent_1960, log_property_value_1960, pct_foreign_born_1960) %>%
  summarise(mean(pct_black_1960, na.rm = T), mean(pct_foreign_born_1960, na.rm = T), mean(pct_owner_occupied_1960, na.rm = T),
            mean(pct_unemployed_males_1960, na.rm = T), mean(log_contract_rent_1960, na.rm = T), 
            mean(log_property_value_1960, na.rm = T), 
            sd(pct_black_1960, na.rm = T), sd(pct_foreign_born_1960, na.rm = T), sd(pct_owner_occupied_1960, na.rm = T),
            sd(pct_unemployed_males_1960, na.rm = T), sd(log_contract_rent_1960, na.rm = T), 
            sd(log_property_value_1960, na.rm = T)) %>%
  view()

# only redlined
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined == 1, !(is.na(predicted_pct_d))) %>%
  select(pct_black_1960, pct_owner_occupied_1960, pct_unemployed_males_1960, log_contract_rent_1960, log_property_value_1960, pct_foreign_born_1960) %>%
  summarise(mean(pct_black_1960, na.rm = T), mean(pct_foreign_born_1960, na.rm = T), mean(pct_owner_occupied_1960, na.rm = T),
            mean(pct_unemployed_males_1960, na.rm = T), mean(log_contract_rent_1960, na.rm = T), 
            mean(log_property_value_1960, na.rm = T), 
...
test_tracts_selected %>%
  filter(selected_tracts == 1) %>%
  filter(redlined != 1 | (redlined == 1 & is.na(predicted_pct_d))) %>%
  select(pct_black_1960, pct_owner_occupied_1960, pct_unemployed_males_1960, log_contract_rent_1960, log_property_value_1960, pct_foreign_born_1960) %>%
  summarise(mean(pct_black_1960, na.rm = T), mean(pct_foreign_born_1960, na.rm = T), mean(pct_owner_occupied_1960, na.rm = T),
            mean(pct_unemployed_males_1960, na.rm = T), mean(log_contract_rent_1960, na.rm = T), 
            mean(log_property_value_1960, na.rm = T), 
