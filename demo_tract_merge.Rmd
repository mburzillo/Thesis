...
data_for_analysis$pct_contract_rent_lt_50 <- 100 * (data_for_analysis$contract_rent_lt_20 + data_for_analysis$contract_rent_20_29 + data_for_analysis$contract_rent_30_39
                                              + data_for_analysis$contract_rent_40_49)/(data_for_analysis$contract_rent_lt_20 + data_for_analysis$contract_rent_20_29 +
                                                                                          ...
                                                                                          data_for_analysis$contract_rent_no_cash_rent)
colnames(data_for_analysis)

data_for_analysis <- data_for_analysis %>%
  select(NHGISST:AREANAME, tot_persons_printed:pct_contract_rent_lt_50)
...
mean1 <- mean(summ$pct_grade_d)

summ <- data_for_analysis %>% 
  filter(area_accounted_for >= .5, citypop_total > 60000) %>%
  mutate(sqd_diff_from_mean = (pct_grade_d - mean1)^2)

mean(summ$pct_grade_d)
var(summ$pct_grade_d)

summ %>%
  select(pct_grade_d, sqd_diff_from_mean)

mean(summ$sqd_diff_from_mean)

summ %>%
  ggplot(aes(x = sqd_diff_from_mean)) +
  geom_histogram()
